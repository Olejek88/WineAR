...
import ru.shtrm.ariant.SampleAppRenderer;
import ru.shtrm.ariant.SampleAppRendererControl;
import ru.shtrm.ariant.utils.SampleMath;
import ru.shtrm.ariant.utils.SampleUtils;
...
public class VideoPlaybackRenderer implements GLSurfaceView.Renderer, SampleAppRendererControl
{
    ...
    SampleAppRenderer mAriantAppRenderer;
    ...
    public VideoPlaybackRenderer(VideoPlayback activity,
        AriantApplicationSession session)
    {
        
        mActivity = activity;
        vuforiaAppSession = session;

        // AriantAppRenderer used to encapsulate the use of RenderingPrimitives setting
        // the device mode AR/VR and stereo mode
        mAriantAppRenderer = new SampleAppRenderer(this, mActivity, Device.MODE.MODE_AR, false, 10f, 5000f);
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config)
    {
        ...
        mAriantAppRenderer.onSurfaceCreated();
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height)
    {
        ...
        mAriantAppRenderer.onConfigurationChanged(mIsActive);
    ...
    public void onDrawFrame(GL10 gl)
    {
        ...
        // Call our function to render content from AriantAppRenderer class
        mAriantAppRenderer.render();
    ...
    public void setActive(boolean active)
    {
        ...
        if(mIsActive)
            mAriantAppRenderer.configureVideoBackground();
    }
    ...
    void initRendering()
    {
        ...
        keyframeQuadAspectRatio[VideoPlayback.RKATSITELI] = (float) mTextures
            .get(0).mHeight / (float) mTextures.get(0).mWidth;
        keyframeQuadAspectRatio[VideoPlayback.MIRELI] = (float) mTextures
                .get(1).mHeight / (float) mTextures.get(1).mWidth;
    ...
    @SuppressLint("InlinedApi")
    // The render function called from AriantAppRendering by using RenderingPrimitives views.
    // The state is owned by AriantAppRenderer which is controlling it's lifecycle.
    // State should not be cached outside this method.
    public void renderFrame(State state, float[] projectionMatrix)
    {
        // Renders video background replacing Renderer.DrawVideoBackground()
        mAriantAppRenderer.renderVideoBackground();
        ...
        for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++)
        {
            ...
            if (imageTarget.getName().compareTo("rkatsiteli") == 0)
                currentTarget = VideoPlayback.RKATSITELI;
            else
                currentTarget = VideoPlayback.MIRELI;
            ...
            // the actual contents
            {
                ...
                if (imageTarget.getName().compareTo("rkatsiteli") == 0)
                    GLES20.glVertexAttribPointer(videoPlaybackTexCoordHandle,
                        ...
                        fillBuffer(videoQuadTextureCoords_1));
                else
                    GLES20.glVertexAttribPointer(videoPlaybackTexCoordHandle,
                            2, GLES20.GL_FLOAT, false, 0,
                            fillBuffer(videoQuadTextureCoords_2));
        ...
        float[] textureCoordMatrix)
    {
        ...
        if (target == VideoPlayback.RKATSITELI)
        {
